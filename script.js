...
window.checkStep1Completion = () => {
    ...
    let errorMessage = '';
    if (!isNameValid) errorMessage = t('errorNameRequired');
    else if (!isWhatsappValid) errorMessage = t('errorWhatsapp');
    else if (!isParentPhoneValid) errorMessage = t('errorParentPhone');
...
window.checkStep2Completion = () => {
    ...
    if (level !== '' && grade !== '') {
        ...
        nextBtn.style.pointerEvents = 'none';
        errorDiv.style.display = 'block';
        errorDiv.innerHTML = t('errorLevelGrade');
        return false;
    }
};
...
window.handleRegisterEmail = async () => {
    if (!checkStep1Completion() || !checkStep2Completion()) {
        return alert(t('error') + " ❌ " + t('completeData'));
    }
    ...
    if (!email || !pass) return alert(t('error') + " ❌ " + t('enterEmailPass'));
    if (pass.length < 6) return alert(t('error') + " ❌ " + t('passMin6'));
    if (pass !== passConfirm) return alert(t('error') + " ❌ " + t('passNotMatch'));
    ...
    try {
        ...
        alert(t('success') + " ✅ " + t('registerSuccess') + " " + t('studentId') + ": " + sid); 
        closeLogin();
    } catch(err) {
        alert(t('error') + " ❌ " + (err.message || err.toString()));
    } finally {
        btn.disabled = false;
...
window.handleRegisterUsername = async () => {
    ...
    if (!username || !pass) return alert(t('error') + " ❌ " + t('enterUsernamePass'));
    if (pass.length < 6) return alert(t('error') + " ❌ " + t('passMin6'));
    if (pass !== passConfirm) return alert(t('error') + " ❌ " + t('passNotMatch'));
...
window.registerWithGoogle = async () => {
    if (!checkStep1Completion() || !checkStep2Completion()) {
        return alert(t('error') + " ❌ " + t('completeDataFirst'));
    }
...
window.loginEmailSubmit = async () => {
    ...
    if(!e || !p) return alert(t('error') + " " + t('enterData'));
    
    try {
        await signInWithEmailAndPassword(auth, e, p);
        closeLogin();
    } catch(err) {
        alert(t('error') + " " + t('loginError'));
    }
};
...
window.loginUsernameSubmit = async () => {
    ...
    if(!username || !pass) return alert(t('error') + " " + t('enterData'));
...
function loadFolders() {
    onValue(ref(db, 'folders'), async (snapshot) => {
        ...
        if (grid.children.length === 0) {
            grid.innerHTML = "<p style='text-align:center; grid-column:1/-1;'>" + t('noCourses') + "</p>";
        }
    });
}
...
window.loadPerfectScores = async () => {
    try {
        ...
        Object.values(students).forEach(student => {
            if (student.shortId) {
                gradeMap[student.shortId] = student.grade || t('gradeNotSpecified');
            }
        });
        ...
        resultsSnap.forEach(result => {
            ...
            if (res.score === res.total && res.score > 0) {
                perfectScores.push({
                    studentName: res.student || t('student'),
                    studentId: res.studentId || '',
                    examName: res.quiz || t('exam'),
                    grade: gradeMap[res.studentId] || t('gradeNotSpecified'),
                    score: res.score,
                    total: res.total,
                    time: res.time || ''
                });
            ...
            finalList.forEach(ps => {
                html += `<div class="perfect-card">
                    ...
                    <div class="perfect-grade">
                        ...
                        ${t('gradeLabel')} ${escapeHTML(ps.grade)}
                    </div>
                    <div class="perfect-score">
                        <i class="fas fa-check-circle"></i> ${t('excellent')} ${ps.score}/${ps.total}
                    </div>
                </div>`;
            });
...
async function checkBadges(uid, totalPoints, studentData) {
    try {
        ...
        if (newBadges.length > 0) {
            ...
            alert(t('congratsBadge') + ' ' + newBadges.map(b => BADGES[b].name).join(', '));
        }
    } catch(error) {
...
window.loadCourseContent = async (folderId, folderName, hasAccess) => {
        ...
        vSnap.forEach(v => {
            ...
            if (!hasAccess) {
                const lockMsg = createElementSafely('span', {
                    ...
                    textContent: t('subscribeToWatch')
                });
    ...
    if (qSnap.exists()) {
        qSnap.forEach(q => {
            ...
            if (quizData.videoRel === "all" || !quizData.videoRel) {
                ...
                if (isCompleted) {
                    badge.textContent = t('reviewQuiz');
                    badge.style.cssText = 'background: var(--success); color: white;';
                } else {
                    badge.textContent = t('startQuiz');
                }
                detailsDiv.appendChild(badge);
                ...
                if (isCompleted) {
                    ...
                    scoreSpan.innerHTML = `<i class="fas fa-check-circle"></i> ${t('score')}: ${examResultsMap[quizId].score}/${examResultsMap[quizId].total} (${examResultsMap[quizId].percentage}%)`;
    ...
    if (grid.children.length === 0) {
        grid.innerHTML = "<p style='text-align:center; grid-column:1/-1;'>" + t('comingSoon') + "</p>";
    }
};
...
async function loadCourseRatingUI(courseId) {
    ...
    if (!userReviewed) {
        ...
        <button onclick="window.submitCourseRating('${courseId}')" class="btn" style="background:var(--main); color:white;">${t('submitReview')}</button>`;
    }
...
window.submitCourseRating = async (courseId) => {
    ...
    if (!rating) return alert(t('selectRating'));
    
    const reviewTextEl = document.getElementById('reviewText');
    const review = reviewTextEl ? reviewTextEl.value : '';
    
    const userSnap = await get(child(dbRef, `students/${currentUser.uid}`));
    const studentName = userSnap.val()?.name || currentUser.displayName || t('student');
...
window.startQuiz = async (folderId, quizId) => {
    ...
    html += `<button onclick="window.submitQuiz('${folderId}', '${quizId}')" style="background:var(--main); color:white; border:none; padding:15px; border-radius:15px; cursor:pointer; font-weight:bold; width:100%; font-size:1.1rem; font-family:'Cairo';">${t('submitAnswers')}</button>`;
...
window.viewQuizResult = async (folderId, quizId) => {
    ...
    if (!quizSnap.exists()) return alert(t('quizNotFound'));
    if (!resultSnap.exists()) return alert(t('resultNotFound'));
    ...
    if (quizTitle) {
        quizTitle.innerHTML = `📝 ${t('reviewQuiz')}: ${quizData.name || ''} <span style="font-size:0.9rem; color:var(--success); margin-right:15px;">${t('score')}: ${resultData.score}/${resultData.total} (${resultData.percentage}%)</span>`;
    }
    if (quizOverlay) quizOverlay.style.display = 'block';
    
    if (!quizContainer) return;

    let html = `<div style="margin-bottom: 20px; color: #666; font-weight: bold;">🔍 ${t('reviewOnly')}</div>`;
    ...
    Object.keys(questions).forEach((qKey, idx) => {
        ...
        if (isCorrect) {
            ...
            html += `<span style="color: var(--danger);"><i class="fas fa-times-circle"></i> ${t('wrong')} (${t('yourAnswer')}: ${userAnswerText})</span>`;
        }
        html += `</div>`;
        html += `</div>`;
    });

    html += `<button onclick="window.closeQuiz()" style="background:var(--dark); color:white; border:none; padding:15px; border-radius:15px; cursor:pointer; font-weight:bold; width:100%; font-size:1.1rem; font-family:'Cairo';">${t('close')}</button>`;
    quizContainer.innerHTML = html;
};
...
window.openDashboard = async () => {
    ...
    if (dashboardStudentId) {
        dashboardStudentId.innerHTML = `<i class="fas fa-id-card"></i> ${t('studentId')}: ${escapeHTML(myShortId)}`;
    }
    ...
    if (recentExamsList) {
        recentExamsList.innerHTML = examsHtml || `<p style="text-align:center; color:#999;">${t('noExams')}</p>`;
    }
    ...
    if (recentVideosList) {
        recentVideosList.innerHTML = videosHtml || `<p style="text-align:center; color:#999;">${t('noVideos')}</p>`;
    }
...
window.loadContinueWatching = async () => {
    ...
    for (const [courseId, subData] of Object.entries(subscriptions)) {
        ...
        const p = createElementSafely('p', {
            style: 'color: var(--main);',
            textContent: '▶️ ' + t('continue') + ': ' + (nextVideo.title || '')
        });
        div1.appendChild(h4);
        div1.appendChild(p);
        card.appendChild(div1);
...
onAuthStateChanged(auth, async user => {
    ...
    if (user) {
        ...
        if (reviewContainer) {
            reviewContainer.innerHTML = `<div class="add-review-box"><h3>${t('writeReview')} 👇</h3><textarea id="stuText" rows="3" placeholder="${t('writeReview')}..."></textarea><button onclick="window.sendStuReview()" style="background:var(--main); color:white; border:none; padding:12px; border-radius:50px; cursor:pointer; font-weight:bold; width:100%;">${t('submitReview')}</button></div>`;
        }
        ...
        isAdminUser = false;
        statusDiv.innerHTML = `<button class="auth-btn" onclick="window.openLogin()" style="background:var(--main); color:white; border:none; padding:8px 20px; border-radius:10px; font-weight:bold; cursor:pointer;">${t('login')}</button>`;
        
        if (reviewContainer) {
            reviewContainer.innerHTML = `<div class="review-locked"><i class="fas fa-lock"></i> ${t('loginToReview')}</div>`;
        }
...
window.updateGrades = () => {
    ...
    if (level && grades[level]) {
        ...
        gradeSelect.innerHTML = `<option value="">${t('selectLevelFirst')}</option>`;
    }
